...
class Job():
    ...
    def read_from_file(self, path):
        ...
        with open(path + '/job', 'r') as job_file:
            ...
            partition = re.findall("partition=([\w|/ | \.]+)", lines)
            self.partition =  'ampere' if partition == [] else partition[0]

            checkpoint_path = re.findall("checkpoint=([\w|/ | \.]+)", lines)
            self.checkpoint_path = None if checkpoint_path == [] else checkpoint_path[0]

            id = re.findall("id=([\w|/ | \.]+)", lines)
    ...
    def update_job_file(self):
        ...
        if self.partition != 'ampere':
            lines=lines + ['partition='+self.partition+'\n']
...
class JobMaster():
    ...
    def get_checkpoint_path(self, job):
        ...
        chpt_path =  os.path.join(self.repo_path, job.log, job.name,'checkpoints', 'best', 'last.ckpt') 
        return chpt_path
