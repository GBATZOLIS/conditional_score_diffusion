...
from lightning_modules import DebugHSICmodule, HSICDisentangledScoreVAEmodel, No_MI_DisentangledScoreVAEmodel, DisentangledScoreVAEmodel, AttributeConditionalModel, AttributeEncoder, BaseSdeGenerativeModel, ScoreVAEmodel, PretrainedScoreVAEmodel, EncoderOnlyPretrainedScoreVAEmodel, CorrectedEncoderOnlyPretrainedScoreVAEmodel, ConditionalSdeGenerativeModel #need for lightning module registration
...
def test(config, log_path, checkpoint_path):
    ...
    if config.eval.test_mode == 'HSIC':
        scoreVAE_testing.calculate_hsic(logger)
    elif config.eval.test_mode == 'correlation':
        scoreVAE_testing.test_correlation_matrix(logger)
    else:
      if config.data.create_dataset:
          create_dataset.create_dataset(config)
      ...
      if config.eval.test_mode == 'generate_latents':
          scoreVAE_testing.generate_latents(pl_module, DataModule, logger)
      elif config.eval.test_mode == 'test':
          ...
          raise ValueError(f"Unknown test mode: {config.eval.test_mode}")
