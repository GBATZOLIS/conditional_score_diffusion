...
def get_config():
  ...
  training.sde = 'vesde'
  # callbacks
  training.visualization_callback = 'Conditional2DVisualization'
  ...
  data.datamodule = 'ConditionalSynthetic'
  data.dataset_type = 'ConditionalGaussianBubbles'
  data.use_data_mean = False # What is this?
  data.create_dataset = False
  data.split = [0.8, 0.1, 0.1]
  data.data_samples = int(5e4)
  data.mixtures = 4 
  data.y_min = 0
  data.y_max = 2
  ...
  model.sigma_max_y_target = np.sqrt(np.prod(data.shape_y))
